...
import type { ExecutionPlan, ReflectionEntry } from "../../types/index.js";
import { nowMs } from "../../utils/index.js";
import type { ChatMessage, InferenceEngine } from "../engine/index.js";
...
export class ReflectionEngine {
  ...
  constructor(inferenceEngine: InferenceEngine, maxDepth = 3) {
  ...
  async reflect(
    ...
    try {
      ...
      log.info(
        `Reflection on step ${stepId}: quality=${parsed.quality}, issues=${parsed.issues.length}, retry=${parsed.shouldRetry}`,
      );
  ...
  async reflectOnPlan(plan: ExecutionPlan): Promise<ReflectionResult> {
