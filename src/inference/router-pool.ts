...
export class PoolIntegratedRouter {
  ...
  decide(params: {
    ...
    if (needsVision) {
      ...
      if (vl)
        return {
          model: vl,
          provider: this.modelConfigs.get(vl)!.provider,
          reason: "Vision task",
          confidence: 0.95,
    ...
    if (mode === "deep") {
      ...
      if (big)
        return {
          model: big,
          provider: this.modelConfigs.get(big)!.provider,
          reason: "Deep mode",
          confidence: 0.9,
    ...
    const fast =
      [...this.modelConfigs.keys()].find(
        (m) => m.includes("4b") && !m.includes("vl"),
      ) || [...this.modelConfigs.keys()][0]!;
    return {
      model: fast,
      provider: this.modelConfigs.get(fast)!.provider,
      reason: "Default fast",
      confidence: 0.7,
      estimatedLatency: 300,
    };
  ...
  async infer(
    ...
    if (!config)
      return {
        success: false,
        error: `Model ${decision.model} not configured`,
        latency: 0,
      };
