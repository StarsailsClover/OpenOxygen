...
export function registerKeyPool(provider: string, keys: string[]): void {
...
export class ModelRouter {
  ...
  route(mode: InferenceMode, constraints?: RoutingConstraints): RoutingDecision | null {
    ...
    log.debug(
      `Routed to ${best.model.provider}/${best.model.model} (score: ${best.score.toFixed(2)}, mode: ${mode})`,
    );
  ...
  private filterCandidates(constraints?: RoutingConstraints): ModelConfig[] {
    let candidates = [...this.models];

    if (constraints?.excludeProviders) {
      candidates = candidates.filter(
        (m) => !constraints.excludeProviders!.includes(m.provider),
      );
    }
  ...
  private scoreModel(
    ...
    if (!profile) return 5; // Neutral score for unknown models
    ...
    return (
      profile.reasoning * weights.reasoning +
