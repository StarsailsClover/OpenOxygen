...
export interface ElementSelector {
    type: "automationId" | "name" | "className" | "controlType" | "text" | "xpath";
    value: string;
    partial?: boolean;
}
...
export class UIAElementDetector {
    ...
    async clickElement(element: UIAElement): Promise<boolean> {
        log.info(`Clicking element: ${element.name} at (${element.center.x}, ${element.center.y})`);
     ...
     * 等待元素出现
     */
    async waitForElement(selector: ElementSelector, timeoutMs: number = 5000): Promise<UIAElement | null> {
        log.info(`Waiting for element: ${selector.value} (timeout: ${timeoutMs}ms)`);
