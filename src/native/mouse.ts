 ...
 */
function mouseMovePowerShell(x: number, y: number): boolean {
  try {
 ...
 */
export function mouseClick(button: keyof typeof MouseButton = "LEFT"): boolean {
 ...
 */
function mouseClickPowerShell(button: keyof typeof MouseButton): boolean {
  try {
    const { execSync } = require("node:child_process");
 ...
 * @param button - Mouse button (LEFT, RIGHT, MIDDLE)
 */
export function mouseDoubleClick(button: keyof typeof MouseButton = "LEFT"): boolean {
...
export function mouseDrag(
  ...
  endX: number,
  endY: number,
  button: keyof typeof MouseButton = "LEFT"
): boolean {
...
function mouseDragPowerShell(
  ...
  endX: number,
  endY: number,
  button: keyof typeof MouseButton
): boolean {
  try {
    const { execSync } = require("node:child_process");
 ...
 */
export function mouseScroll(delta: number): boolean {
 ...
 */
function mouseScrollPowerShell(delta: number): boolean {
  try {
    const { execSync } = require("node:child_process");
    const script = `
      Add-Type @"
