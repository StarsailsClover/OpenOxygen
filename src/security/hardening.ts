 ...
 */
export function sanitizeGatewayUrl(input: string): string | null {
  try {
...
// ═══════════════════════════════════════════════════════════════════════════
// 2. WEBSOCKET ORIGIN VALIDATION + RATE LIMITING  (ClawJacked)
//    OpenClaw 的 WebSocket 不校验 Origin，任何网页都能连接 localhost。
//    OpenOxygen 方案：严格 Origin 白名单 + 连接速率限制 + 设备审批。
// ═══════════════════════════════════════════════════════════════════════════
...
export function validateWebSocketOrigin(
  ...
  if (!origin) return false; // 无 Origin 一律拒绝
