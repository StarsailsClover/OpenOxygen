...
import { connectCDP, executeScriptCDP, clickElementCDP, queryElementCDP } from "./cdp.js";
...
export async function typeTextHumanLike(
  ...
  try {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      if (opts.humanLikeTyping && opts.typingErrorRate && Math.random() < opts.typingErrorRate) {
...
export async function detectAndFillForm(
  ...
  const patterns: Record<string, string[]> = {
    username: ['input[name*="user"]', 'input[name*="login"]', 'input[id*="user"]', 'input[id*="login"]', 'input[type="text"]:first-of-type'],
    email: ['input[type="email"]', 'input[name*="email"]', 'input[id*="email"]', 'input[placeholder*="email" i]'],
    password: ['input[type="password"]', 'input[name*="pass"]', 'input[id*="pass"]', 'input[name*="pwd"]', 'input[id*="pwd"]'],
    search: ['input[type="search"]', 'input[name*="search"]', 'input[id*="search"]', 'input[placeholder*="search" i]'],
    phone: ['input[type="tel"]', 'input[name*="phone"]', 'input[name*="tel"]', 'input[id*="phone"]'],
  };
  ...
  for (const [key, value] of Object.entries(formData)) {
    ...
    for (const selector of selectors) {
      try {
        const exists = await executeScriptCDP(cdp, `
