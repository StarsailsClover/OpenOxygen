...
export class HumanLikenessScorer {
  ...
  private scoreTiming(actions: Array<{ timestamp: number }>): TimingScore {
    ...
    // 评分
    let score = 50;
    if (hasNaturalVariance) score += 20;
    if (hasMicroPauses) score += 15;
    if (mean > 50 && mean < 2000) score += 15; // 合理的间隔范围

    // 惩罚
    if (cv < 0.05) score -= 30; // 太均匀 = 机器人
    if (mean < 10) score -= 20; // 太快
  ...
  private scoreMovement(
    ...
    let score = 50;
    if (pathSmoothness > 0.5) score += 15;
    if (hasAcceleration) score += 15;
    if (hasOvershoot) score += 10;
    if (straightLineRatio < 0.3) score += 10;
    if (straightLineRatio > 0.8) score -= 20; // 太直 = 机器人
  ...
  private scorePattern(
    ...
    if (hasHesitation) score += 15;
    if (clickPrecision < 0.8) score += 10;
    if (isRepetitive) score -= 25;
