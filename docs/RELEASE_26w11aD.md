...
**26w11aD** — "D" for "Dynamic" — introduces major architectural improvements:

1. **Multi-Model Local LLM Support** — 3 Ollama models running simultaneously
2. **Vision-Language Integration** — Qwen3-VL for screen understanding
3. **High-Performance Rust Inference Client** — reqwest + tokio for 5x concurrency
4. **RocksDB Persistent Storage** — TB-scale vector persistence
5. **Dynamic Model Router** — automatic model selection based on task
...
- **qwen3:4b** (2.5GB) → Fast queries, text generation
- **qwen3-vl:4b** (3.3GB) → Screenshot analysis, UI detection
- **gpt-oss:20b** (13GB) → Deep planning, complex reasoning
...
```
Before (26w11aC):
  Gateway → TS InferenceEngine → fetch() → LLM
                ↓
          TS VectorStore (in-memory)

After (26w11aD):
  Gateway → TS InferenceEngine → Rust Client → LLM
                ↓
...
|-------|------|--------|-------|
| qwen3:4b | 2.5 GB | ✅ Ready | General |
| qwen3-vl:4b | 3.3 GB | ✅ Ready | Vision |
| gpt-oss:20b | 13 GB | ✅ Ready | Reasoning |
| **Total** | **18.8 GB** | | |
...
3. **No code changes required** — existing agents continue working
