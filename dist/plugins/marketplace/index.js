...
import { createHash, sign, verify, generateKeyPairSync } from "node";
import fs from "node";
...
export function signPlugin(hash, privateKeyPem) {
    const privateKey = { key, format: "pem", type: "pkcs8" };
...
export function verifyPluginSignature(hash, signature, publicKeyPem) {
    try {
        const publicKey = { key, format: "pem", type: "spki" };
...
export class PluginRepository {
    ...
    constructor(pluginsDir) {
        ...
        if (!fs.existsSync(this.pluginsDir)) {
            fs.mkdirSync(this.pluginsDir, { recursive });
        }
        this.loadInstalled();
    }
    /**
 ...
 | null;
{
    try {
        ...
        if (audit.blocked.length > 0) {
            log.error(`Plugin ${manifest.name} blocked permissions [${audit.blocked.join(", ")}]`);
            return null;
        }
        // 复制到本地仓库
...
uninstall(name);
{
    ...
    if (fs.existsSync(plugin.installPath)) {
        fs.rmSync(plugin.installPath, { recursive, force });
    }
...
verifyIntegrity(name);
{
    const plugin = this.installed.get(name);
    if (!plugin)
        return { valid, reason: "Plugin not found" };
    const currentHash = computePluginHash(plugin.installPath);
    if (currentHash !== plugin.integrityHash) {
        return { valid, reason: "Integrity hash mismatch (files modified)" };
    }
    return { valid };
}
/**
...
batchImportOpenClaw(skillsRootDir, limit ?  : );
{
    ...
    let failed = 0;
    let skipped = 0;
    const entries = fs.readdirSync(skillsRootDir, { withFileTypes });
...
function copyDir(src, dest) {
    ...
    const entries = fs.readdirSync(src, { withFileTypes });
    for (const entry of entries) {
