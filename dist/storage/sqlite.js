...
export class SQLiteStore {
    ...
    saveSession(session) {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `)
            .run(session.key, session.agentId, session.channelId || null, session.createdAt, session.lastActiveAt, JSON.stringify(session.metadata || {}));
    }
    getSession(key) {
    ...
    saveChunk(chunk) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)
            .run(chunk.id, chunk.source, chunk.filePath, chunk.startLine, chunk.endLine, chunk.contentHash, chunk.chunkSize, chunk.createdAt, chunk.expiresAt || null, chunk.embeddingDim || 0);
    }
    getChunksByPath(filePath) {
        return this.db
    ...
    deleteExpiredChunks() {
        const result = this.db
            .prepare("DELETE FROM vector_chunks WHERE expires_at IS NOT NULL AND expires_at < ?")
            .run(Date.now());
    ...
    getStats() {
        const sessions = this.db.prepare("SELECT COUNT(*) as c FROM sessions").get()?.c ||
            0;
        const audit = this.db.prepare("SELECT COUNT(*) as c FROM audit_log").get()
            ?.c || 0;
        const chunks = this.db.prepare("SELECT COUNT(*) as c FROM vector_chunks").get()
            ?.c || 0;
        const kv = this.db.prepare("SELECT COUNT(*) as c FROM kv_store").get()?.c ||
            0;
        const stats = this.db.prepare("SELECT COUNT(*) as c FROM model_stats").get()
            ?.c || 0;
        let dbSize = 0;
        try {
