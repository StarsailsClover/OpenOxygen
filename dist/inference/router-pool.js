...
export class PoolIntegratedRouter {
    ...
    decide(params) {
        ...
        if (needsVision) {
            ...
            if (vl)
                return { model: vl, provider: this.modelConfigs.get(vl).provider, reason: "Vision task", confidence: 0.95, estimatedLatency: 500 };
        }
        if (mode === "deep") {
            const big = [...this.modelConfigs.keys()].find(m => m.includes("20b"));
            if (big)
                return { model: big, provider: this.modelConfigs.get(big).provider, reason: "Deep mode", confidence: 0.9, estimatedLatency: 2000 };
        }
    ...
    async infer(decision, request) {
        ...
        if (!config)
            return { success: false, error: `Model ${decision.model} not configured`, latency: 0 };
        try {
