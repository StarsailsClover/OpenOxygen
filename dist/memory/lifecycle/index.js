...
function chunkText(text, source, filePath, chunkSize = DEFAULT_CHUNK_SIZE, overlap = DEFAULT_CHUNK_OVERLAP) {
...
async function indexFile(filePath, source) { }
<MemoryChunk />;
[] > {
    try: {
...
async function indexDirectory(dirPath, source, maxDepth = 5, currentDepth = 0) { }
<MemoryChunk />;
[] > {
    if(currentDepth) { }
} >= maxDepth;
...
export class MemoryManager {
 ...
 < { indexed, chunks } > {
    ...
    if(, opts, force) { }
} && nowMs() - this.lastSyncTime < 60_000;
{
...
for (let i = 0; i < paths.length; i++) {
    const p = paths[i];
    opts?.progress?.({ completed, total, : .length, label });
    try {
...
search(query, opts ?  : ) < MemorySearchResult[] > {
    // TODO query embedding via configured embedding provider
...
status();
{
    return {
        ...
        indexedPaths, : .indexedPaths.size,
