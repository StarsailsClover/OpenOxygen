...
import { existsSync, mkdirSync } from "node";
...
// ─── Task History ─────────────────────────────────────────────────────────
recordTask(task < TaskRecord, "id" | "createdAt" > );
{
    ...
    const stmt = this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
    stmt.run(id, task.instruction, task.mode, task.success ? 1 : , task.output || null, task.error || null, task.durationMs, createdAt);
    // Index context
    if (task.metadata) {
        ...
        VALUES (?, ?, ?, ?)
      `);
        indexStmt.run(id, task.metadata.app || null, task.metadata.url || null, task.metadata.keywords ? JSON.stringify(task.metadata.keywords)
            :
        );
    }
...
queryTasks(query);
{
    ...
    return rows.map(row => ({
        ...
        mode, : .mode,
        success(row) { }, : .success
    }), output.output, error.error, durationMs.duration_ms, createdAt.created_at);
}
;
...
injectContext(instruction);
{
    ...
    for (const keyword of keywords) {
        const related = this.queryTasks({ keyword, limit });
...
getStats();
{
    const totalStmt = this.db.prepare("SELECT COUNT(*)  FROM task_history");
    const { count } = totalStmt.get();
    const successStmt = this.db.prepare("SELECT COUNT(*)  FROM task_history WHERE success = 1");
    const { count } = successStmt.get();
    const durationStmt = this.db.prepare("SELECT AVG(duration_ms)  FROM task_history");
    const { avg } = durationStmt.get();
    const modeStmt = this.db.prepare("SELECT mode, COUNT(*)  FROM task_history GROUP BY mode");
    const modeRows = modeStmt.all() < { mode, count } > ;
    const byMode = {};
