...
async function waitForCDP(session, port, timeoutMs = 30000) {
...
async function inheritSystemCookies(targetDir) {
...
export async function getPageInfo(sessionId) {
    const session = sessions.get(sessionId);
    if (!session?.cdpClient)
        return null;
    try {
...
export async function querySelectorAll(sessionId, selector) {
    const session = sessions.get(sessionId);
    if (!session?.cdpClient)
        return [];
    try {
...
async function findElementByText(sessionId, text) {
    const session = sessions.get(sessionId);
    if (!session?.cdpClient)
        return null;
    try {
...
async function findElementByUIA(sessionId, name) {
    // Import native module for UIA fallback
    try {
        const native = require("../../native-bridge.js").loadNativeModule();
        if (!native?.getUiElements)
            return null;
        const elements = native.getUiElements(null);
        const found = elements.find((e) => e.name?.toLowerCase().includes(name.toLowerCase()) ||
...
export async function typeText(sessionId, selector, text) {
    const session = sessions.get(sessionId);
    if (!session?.cdpClient) {
        return { success: false, error: "Session not available", durationMs: 0 };
    }
