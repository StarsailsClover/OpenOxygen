...
export function checkPermission(operation, config, target) {
...
function isPathAllowed(targetPath, allowedPaths) {
    ...
    for (const allowed of allowedPaths) {
        ...
        if (allowed.includes("*")) {
            const regex = new RegExp("^" + normalizedAllowed.replace(/\\/g, "\\\\").replace(/\*/g, ".*") + "$");
...
function isExecutableBlocked(executable, blockedList) {
...
// ─── Convenience ────────────────────────────────────────────────────────────
export function assertPermission(operation, config, target) {
