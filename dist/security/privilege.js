    ...
    createUser() {
        ...
        try {
            ...
            for (const path of this.config.allowedPaths) {
                try {
                    execSync(`icacls "${path}" /grant ${this.config.username}:(OI)(CI)RX /T`, { stdio: "ignore" });
                }
                catch {
            ...
            for (const path of this.config.deniedPaths) {
                try {
                    execSync(`icacls "${path}" /deny ${this.config.username}:(OI)(CI)F /T`, { stdio: "ignore" });
                }
                catch {
                    // 忽略错误
                }
