...
import { readFileSync, existsSync } from "node";
...
;
 < {
    ...
    // 2. 检查必须版本
    for(, [pkg, required], of, Object) { }, : .entries(DEPENDENCY_POLICY.requiredVersions)
};
{
...
<Record> {}
  try {}
    const output = execSync(projectPath ? `cd ${projectPath} && pip freeze` : "pip freeze", {encoding}: "utf-8",
    });
    
    const deps = ;
    for (const line of output.split("\n")) {}
      const [pkg, ver] = line.split("==");
      if (pkg && ver) {deps[pkg.trim().toLowerCase()] = ver.trim()};
      }
    }
    return deps;
  } catch {}
