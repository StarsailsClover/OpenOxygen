...
export class ThoughtRouter {
     ...
     * 为子任务选择最优模型
     */
    selectModelForRole(role, availableModels) {
        // 启发式选择
        const modelScores = availableModels.map(m => {
            let score = 0;
            if (m.includes("gpt-oss:20b"))
                score += role === "reasoner" ? 3 : 2;
            if (m.includes("qwen3-vl"))
                score += role === "verifier" ? 2 : 1;
            if (m.includes("qwen3:4b"))
                score += role === "critic" ? 2 : 1;
...
export class ConsensusEngine {
    ...
    bestAnswer(nodes) {
        const best = nodes.reduce((max, node) => node.confidence > max.confidence ? node : max);
...
export class ReflectionLoop {
     ...
     * 迭代反思优化
     */
    async iterate(initialAnswer, criticFn, improveFn) {
...
export class AIThinkingCluster {
    ...
    consensus;
    reflection;
    constructor(inferenceEngine, options) {
    ...
    async think(goal, context) {
        ...
        if (analysis.strategy === "single") {
            ...
            // 并行推理 + 综合
            const models = analysis.roles.map(r => this.router.selectModelForRole(r, this.inferenceEngine.getAvailableProviders()));
            const nodes = await Promise.all(models.map((m, i) => this.executeNode(goal, m, analysis.roles[i])));
            ...
            // 反思优化
            const reflection = await this.reflection.iterate(initial.output, async (answer) => {
                const critique = await this.executeNode(`Critique this answer: "${answer}"`, this.router.selectModelForRole("critic", this.inferenceEngine.getAvailableProviders()), "critic");
                return { critique: critique.output, score: critique.confidence };
            }, async (answer, critique) => {
                const improved = await this.executeNode(`Improve this answer based on critique:\nAnswer: ${answer}\nCritique: ${critique}`, this.router.selectModelForRole("synthesizer", this.inferenceEngine.getAvailableProviders()), "synthesizer");
    ...
    async executeNode(input, model, role) {
        const startTime = nowMs();
